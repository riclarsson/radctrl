...
void test002() {
  ...
  std::cout << '\n';
  {
    std::cout << "pos converted from spherical\n";
    auto xs =
        Geom::Pos<Geom::PosType::Spherical>({2.38591e+07, 15.4519, 33.6901});
    auto x = Geom::Pos<Geom::PosType::Xyz>(xs, wgs84);
    auto xe = Geom::Pos<Geom::PosType::Ellipsoidal>{xs, wgs84};
    std::cout << "some-pos (xyz): " << x << '\n';
    std::cout << "some-pos (sph): " << xs << '\n';
  ...
  std::cout << '\n';
  {
    std::cout << "pos converted from ellipsoidal\n";
    auto xe =
        Geom::Pos<Geom::PosType::Ellipsoidal>({1.74825e+07, 15.4783, 33.6901});
    auto x = Geom::Pos<Geom::PosType::Xyz>(xe, wgs84);
    auto xs = Geom::Pos<Geom::PosType::Spherical>(xe, wgs84);
    std::cout << "z-pos (xyz): " << x << '\n';
    std::cout << "z-pos (sph): " << xs << '\n';
...
void test006() {
  ...
  File::File<File::Operation::Write, File::Type::Xml> fraw(
      "test_geom_test001.xml");
  Geom::saveNav(fraw, n);
  fraw.close();

  File::File<File::Operation::WriteBinary, File::Type::Xml> fbin(
      "test_geom_test002.xml");
  Geom::saveNav(fbin, n);
  fbin.close();

  Geom::Nav nraw;
  File::File<File::Operation::Read, File::Type::Xml> frawread(
      "test_geom_test001.xml");
  std::cout << "Print1: " << n << '\n';
  ...
  Geom::Nav nraw2;
  File::File<File::Operation::ReadBinary, File::Type::Xml> fbinread(
      "test_geom_test002.xml");
  std::cout << "Print1: " << n << '\n';
