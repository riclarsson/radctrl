...
public:
  ...
  Point(decltype(vmr)::size_type n=0) noexcept : M({0,0,0}), W({0,0,0}), vmr(n) {}
  Point(const std::vector<Species::Isotope>& s) noexcept : M({0,0,0}), W({0,0,0}), vmr(s.size()) {
  ...
  const std::vector<VMR<VMRType::ratio>>& VolumeMixingRatios() const {return vmr;}
  ...
  friend void savePoint(File::File<File::Operation::Write, File::Type::Xml>& file, const Point& ap) {
  ...
  friend void savePoint(File::File<File::Operation::WriteBinary, File::Type::Xml>& file, const Point& ap) {
    file.new_child("Point");
    file.add_attribute("size", ap.size());
    file.add_attribute("Species", ap.specs());
    ap.saveBinary(file);
  ...
  friend void readPoint(File::File<File::Operation::Read, File::Type::Xml>& file, Point& ap) {
  ...
  friend void readPoint(File::File<File::Operation::ReadBinary, File::Type::Xml>& file, Point& ap) {
...
public:
  ...
  typename std::remove_reference<decltype(ap.P)>::type P() const noexcept {auto out = std::log(ap.P); return out *= scale;}
...
class Atm {
  ...
  bool ok() const {
    ...
    for (auto& x: data.data()) {
      if (specs not_eq x.specs())
        return false;
    }
    return true;
  }
public:
  Atm(const std::vector<Time>& t, const std::vector<Altitude<AltitudeType::meter>>& a,
      const std::vector<Coordinate<CoordinateType::lat>>& la, const std::vector<Coordinate<CoordinateType::lon>>& lo, 
      const Grid<Point, 4>& d) : tid(t), alt(a), lat(la), lon(lo), data(d) {if (not ok()) throw std::runtime_error("Bad atmosphere");}
  Atm(std::size_t t=0, std::size_t a=0, std::size_t la=0, std::size_t lo=0, std::size_t s=0) : tid(t), alt(a), lat(la), lon(lo), data(s, {t,a,la,lo}) {}
      ...
      for (decltype(a.alt.size()) j=0; j<a.alt.size(); j++) {
        for (decltype(a.lat.size()) k=0; k<a.lat.size(); k++) {
          for (decltype(a.lon.size()) m=0; m<a.lon.size(); m++) {
            os << a.tid[i] << ' ' << a.alt[j] << ' ' << a.lat[k] << ' ' << a.lon[m] << ' ' << a.data(i, j, k, m);
            if (i<a.tid.size()-1 or j<a.alt.size()-1 or k<a.lat.size()-1 or m<a.lon.size()-1)
              os << '\n';
          }
        }
      }
  ...
  friend std::istream& operator>>(std::istream& is, Atm& a) {
    ...
    for (decltype(a.tid.size()) i=0; i<a.tid.size(); i++) {
      for (decltype(a.alt.size()) j=0; j<a.alt.size(); j++) {
        for (decltype(a.lat.size()) k=0; k<a.lat.size(); k++) {
          for (decltype(a.lon.size()) m=0; m<a.lon.size(); m++) {
            is >> a.tid[i] >> a.alt[j] >> a.lat[k] >> a.lon[m] >> a.data(i, j, k, m);
          }
        }
      }
    }
  ...
  Point& operator()(size_t i, size_t j, size_t k, size_t m) {return data(i, j, k, m);}
  const Point& operator()(size_t i, size_t j, size_t k, size_t m) const {return data(i, j, k, m);}
  
  Point operator()(Time newtid, Altitude<AltitudeType::meter> newalt, Coordinate<CoordinateType::lat> newlat, Coordinate<CoordinateType::lon> newlon) const;
  template <typename Pos> Point operator()(Pos pos) const {return operator()(pos.t(), pos.h(), pos.lat(), pos.lon());}
  
  friend void saveAtm(File::File<File::Operation::Write, File::Type::Xml>& file, const Atm& a) {
    if (not a.ok()) throw std::runtime_error("Bad atmosphere");
  ...
  friend void saveAtm(File::File<File::Operation::WriteBinary, File::Type::Xml>& file, const Atm& a) {
    if (not a.ok()) throw std::runtime_error("Bad atmosphere");
  ...
  friend void readAtm(File::File<File::Operation::Read, File::Type::Xml>& file, Atm& a) {
    file.get_child("Atm");
  ...
  friend void readAtm(File::File<File::Operation::ReadBinary, File::Type::Xml>& file, Atm& a) {
    file.get_child("Atm");
    a.tid.resize(file.get_attribute("Time").as_int());
    a.alt.resize(file.get_attribute("Altitudes").as_int());
    a.lat.resize(file.get_attribute("Latitudes").as_int());
