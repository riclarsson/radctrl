...
class Rational {
 ...
 public:
  ...
  constexpr bool operator==(Rational r) const noexcept {
    return mn == r.mn and md == r.md;
  }
  constexpr bool operator!=(Rational r) const noexcept {
    return not operator==(r);
  }
  constexpr bool operator<=(Rational other) const {
    return mn * other.md <= other.mn * md;
  }
  constexpr bool operator>=(Rational other) const {
    return mn * other.md >= other.mn * md;
  }
  ...
  friend constexpr Rational operator+(Rational one, int two) {
    return Rational(one.n() + two * one.d(), one.d());
  }
  friend constexpr Rational operator+(int one, Rational two) {
  ...
  friend constexpr Rational operator-(int one, Rational two) {
    return Rational(one * two.d() - two.n(), two.d());
  }
