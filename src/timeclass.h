...
public:
  ...
  // helpers
  TimeStep::rep Seconds() const {return std::chrono::duration_cast<TimeStep>(mtime.time_since_epoch()).count();}
...
inline void Sleep(double duration) {std::this_thread::sleep_for(TimeStep(duration));}
