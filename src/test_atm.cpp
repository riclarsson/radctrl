...
void test003() {
  ...
  File::File<File::Operation::WriteBinary, File::Type::Xml> save("test_atm_test003.xml");
  savePoint(save, a);
  save.close();
  std::cout << "Write: " << a << '\n';
  
  File::File<File::Operation::ReadBinary, File::Type::Xml> read("test_atm_test003.xml");
  readPoint(read, b);
  read.close();
  std::cout << "Read:  " << b << '\n';
}
...
void test004() {
  ...
  Atmosphere::Atm atm({Time(), Time()+TimeStep(1)},
                      {0, 1},
                      {0, 2},
                      {0, 4},
...
void test005() {
  Atmosphere::Point a
                      (100e3,
                       296.,
                       std::array<double, 3>{11e-6, 11e-6,31e-6},
                       std::array<double, 3>{10., 1., 0.1},
                       std::vector<VMR<VMRType::ratio>>{
                         VMR<VMRType::ratio>{Species::Isotope(Species::Species::Nitrogen, 0), 0.78},
                         VMR<VMRType::ratio>{Species::Isotope(Species::Species::Oxygen, 0), 0.2095},
  ...
  Atmosphere::Point b
                      (10e3,
                       292.,
                       std::array<double, 3>{15e-6, 15e-6,35e-6},
                       std::array<double, 3>{15., 2., 0.2},
                       std::vector<VMR<VMRType::ratio>>{
                         VMR<VMRType::ratio>{Species::Isotope(Species::Species::Nitrogen, 0), 0.74},
                         VMR<VMRType::ratio>{Species::Isotope(Species::Species::Oxygen, 0), 0.2055},
                         VMR<VMRType::ratio>{Species::Isotope(Species::Species::Water, 0), 600e-06}}
                      );
  Time t;
  Atmosphere::Atm atm({t, t+TimeStep(1)},
                      {0, 1},
                      {0, 2},
                      {0, 4},
                      Grid<Atmosphere::Point, 4>(a, {2, 2, 2, 2}));
...
void test006() {
  ...
  File::File<File::Operation::Write, File::Type::Xml> save("test_atm_test006.xml");
  saveAtm(save, atm);
  save.close();
  
  Atmosphere::Atm batm;
  File::File<File::Operation::Read, File::Type::Xml> read("test_atm_test006.xml");
  readAtm(read, batm);
  read.close();
...
void test007() {
  ...
  File::File<File::Operation::WriteBinary, File::Type::Xml> save("test_atm_test007.xml");
  saveAtm(save, atm);
  save.close();
  
  Atmosphere::Atm batm;
  File::File<File::Operation::ReadBinary, File::Type::Xml> read("test_atm_test007.xml");
  readAtm(read, batm);
  read.close();
  std::cout << "Read (loaded):\n" <<  batm << '\n';
}
