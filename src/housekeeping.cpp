...
void runonce(std::string& dev, int baud, const std::string& python_file) {
  ...
  hk.startup(dev, baud);
  if (hk.has_error()) {
    std::cerr << "Failure calling hk.startup(\""<<dev<<"\", "<<baud<<").  Error:\n";
...
template <class Housekeeping>
void run(std::string& dev, int baud, const std::string& python_file, const TimeStep dt) {
  if (dt.count() > 7*86400 or dt.count() < 0) {
    std::cerr << "Bad time: " << dt << ".  Allowed range 0 sec to ~ 1 week to avoid under-/overflow\n";
    std::exit(1);
  }
  ...
  for (;;) {
    Time wait_until{Time() + dt};  // Set the wait_until time first in case operations inside are slower than the requested waiting
...
int main(int argc, char ** argv) {
  ...
  if (machine == 0) {
    if (continue_running) {
      run<Instrument::Housekeeping::Agilent34970A>(dev, baud, pythonfile, TimeStep{minimum_waittime});
    } else {
