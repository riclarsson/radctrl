...
constexpr double linear_interpolation (const std::array<double, N>& x, const std::array<double, N>& y, double t) {
  ...
  else if (i == N) return y.back();
  else return y[i-1] + (y[i] - y[i-1]) * (t - x[i-1]) / (x[i] - x[i-1]);
}
...
constexpr double CY7(double v) {
  ...
  constexpr std::array<double,120> Temperature = {  /* in Kelvin */
  ...
  constexpr std::array<double,120> Voltage = {  /* in Volt */
...
public:
  using DataType = std::map<std::string, double>;
  template <typename ... Whatever> Agilent34970A(Whatever...) :
  manual(false), error_found(false), new_data(false), error(""), must_read(true), has_first(false), has_new_data(false)  {}
...
//     std::string name = port.readline();  // This is weird!
  ...
  void run() {
    if (not has_first) {
      ...
      else if ((Time() - T0) > TimeStep(10)) { // It has been more than 10 seconds
        results = port.readline();
        has_new_data = true;
      }
    }
