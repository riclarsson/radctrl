...
public: \
  ...
  constexpr Scalar operator+() const noexcept {return Scalar{val};} \
  template <class T> constexpr T operator+(T x) const noexcept {return val + x;} \
  template <class T> constexpr T operator-(T x) const noexcept {return val - x;} \
  template <class T> constexpr double operator*(T x) const noexcept {return val * x;} \
  template <class T> constexpr double operator/(T x) const noexcept {return val / x;} \
  constexpr double operator+(int x) const noexcept {return val + double(x);} \
  constexpr double operator-(int x) const noexcept {return val - double(x);} \
  constexpr double operator*(int x) const noexcept {return val * double(x);} \
  constexpr double operator/(int x) const noexcept {return val / double(x);} \
  friend constexpr Scalar operator+(int a, Scalar b) noexcept {return Scalar{double(a)+b.val};} \
  friend constexpr Scalar operator-(int a, Scalar b) noexcept {return Scalar{double(a)-b.val};} \
  friend constexpr double operator*(int a, Scalar b) noexcept {return Scalar{double(a)*b.val};} \
  friend constexpr double operator/(int a, Scalar b) noexcept {return Scalar{double(a)/b.val};} \
  friend constexpr Scalar operator+(double a, Scalar b) noexcept {return Scalar{a+b.val};} \
  friend constexpr Scalar operator-(double a, Scalar b) noexcept {return Scalar{a-b.val};} \
  friend constexpr double operator*(double a, Scalar b) noexcept {return Scalar{a*b.val};} \
  friend constexpr double operator/(double a, Scalar b) noexcept {return Scalar{a/b.val};} \
  constexpr Scalar& operator=(Scalar b) noexcept {val = b.val; return *this;} \
  constexpr Scalar& operator=(double b) noexcept {val = b; return *this;} \
  Scalar& operator*=(double x) noexcept {val *= x; return *this;} \
  Scalar& operator+=(Scalar x) noexcept {val += x.val; return *this;} \
  Scalar& operator-=(Scalar x) noexcept {val -= x.val; return *this;} \
  double * operator&() noexcept {return &val;} \
  const double * operator&() const noexcept {return &val;} \
  friend std::ostream& operator<<(std::ostream& os, Scalar s) {return os << s.val;} \
  friend std::istream& operator>>(std::istream& is, Scalar& s) {return is >> s.val;} \
  ...
  constexpr bool operator==(Scalar b) const noexcept {return val == b.val;} \
  constexpr bool operator!=(Scalar b) const noexcept {return val != b.val;} \
  template <class T> constexpr bool operator>(T b) const noexcept {return *this > Scalar{b};} \
  template <class T> constexpr bool operator>=(T b) const noexcept {return *this >= Scalar{b};} \
  template <class T> constexpr bool operator<(T b) const noexcept {return *this < Scalar{b};} \
  template <class T> constexpr bool operator<=(T b) const noexcept {return *this <= Scalar{b};} \
  template <class T> constexpr bool operator==(T b) const noexcept {return *this == Scalar{b};} \
...
public:
  constexpr Pressure(const Pressure<PressureType::Pa>& p) noexcept : val(p.value()) {pa2self();}
  constexpr Pressure(const Pressure<PressureType::Atm>& p) noexcept : val(Conversion::atm2pa(p.value())) {pa2self();}
  constexpr Pressure(const Pressure<PressureType::Bar>& p) noexcept : val(Conversion::bar2pa(p.value())) {pa2self();}
  constexpr Pressure(const Pressure<PressureType::Torr>& p) noexcept : val(Conversion::torr2pa(p.value())) {pa2self();}
};  // Pressure


ENUMCLASS(TemperatureType, char,
          K,
          C,
          F,
          eV
...
class Temperature final {
  ...
  void k2self() noexcept {
    ...
    if constexpr (X == TemperatureType::C) {val=Conversion::k2c(val);}
    if constexpr (X == TemperatureType::F) {val=Conversion::k2f(val);}
    if constexpr (X == TemperatureType::eV) {val=Conversion::ev2k(val);}
  }
public:
  constexpr Temperature(const Temperature<TemperatureType::K>& t) noexcept : val(t.value()) {k2self();}
  constexpr Temperature(const Temperature<TemperatureType::C>& t) noexcept : val(Conversion::c2k(t.value())) {k2self();}
  constexpr Temperature(const Temperature<TemperatureType::F>& t) noexcept : val(Conversion::f2k(t.value())) {k2self();}
  constexpr Temperature(const Temperature<TemperatureType::eV>& t) noexcept : val(Conversion::ev2k(t.value())) {k2self();}
};  // Temperature


ENUMCLASS(MagnetismType, char,
          T,
          G
)  // MagnetismType
...
public:
  constexpr Distance(const Distance<DistanceType::meter>& d) noexcept : val(d.value()) {d2self();}
};  // Distance
...
public:
  constexpr Altitude(const Altitude<AltitudeType::meter>& z) noexcept : val(z.value()) {m2self();}
};  // Altitude
...
public:
  constexpr Length(const Length<LengthType::meter>& z) noexcept : val(z.value()) {m2self();}
};  // Length
...
class Frequency final {
  ...
  void f2self() noexcept {
    ...
    if constexpr (X == FrequencyType::Kayser) {val=Conversion::freq2kaycm(val);}
    if constexpr (X == FrequencyType::Wavelength) {val=Conversion::freq2wavelen(val);}
  }
public:
  constexpr Frequency(const Frequency<FrequencyType::Freq>& f) noexcept : val(f.value()) {f2self();}
  constexpr Frequency(const Frequency<FrequencyType::Kayser>& f) noexcept : val(Conversion::kaycm2freq(f.value())) {f2self();}
  constexpr Frequency(const Frequency<FrequencyType::Wavelength>& f) noexcept : val(Conversion::wavelen2freq(f.value())) {f2self();}
};  // Frequency

ENUMCLASS(AreaType, char,
          m2,
          cm2
)  // AreaType
...
class LineStrength {
  ...
  void hzm22self() noexcept {
    ...
    if constexpr (X == FrequencyType::Kayser and Y == AreaType::m2) {val=Conversion::arts2hitran_linestrength(val);}
  }
public:
  constexpr LineStrength(const LineStrength<FrequencyType::Freq, AreaType::m2>& ls) noexcept : val(ls.value()) {hzm22self();}
  constexpr LineStrength(const LineStrength<FrequencyType::Kayser, AreaType::cm2>& ls) noexcept : val(Conversion::hitran2arts_linestrength(ls.value())) {hzm22self();}
};  // LineStrength

ENUMCLASS(EnergyType, char,
          Joule,
          invcm
)  // EnergyType
...
class Energy {
  ...
  void hzm22self() noexcept {
    ...
    if constexpr (X == EnergyType::invcm) {val=Conversion::arts2hitran_energy(val);}
  }
public:
  constexpr Energy(const Energy<EnergyType::Joule>& e) noexcept : val(e.value()) {hzm22self();}
  constexpr Energy(const Energy<EnergyType::invcm>& e) noexcept : val(Conversion::hitran2arts_energy(e.value())) {hzm22self();}
};  // Energy 
...
class PressureBroadening {
  ...
  void hzpa2self() noexcept {
    ...
    if constexpr (X == FrequencyType::Kayser and Y == PressureType::Atm) {val=Conversion::arts2hitran_broadening(val);}
  }
public:
  constexpr PressureBroadening(const PressureBroadening<FrequencyType::Freq, PressureType::Pa>& pb) noexcept : val(pb.value()) {hzpa2self();}
  constexpr PressureBroadening(const PressureBroadening<FrequencyType::Kayser, PressureType::Atm>& pb) noexcept : val(Conversion::hitran2arts_broadening(pb.value())) {hzpa2self();}
};  // PressureBroadening 
