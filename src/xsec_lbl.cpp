...
double compute_lte_linestrength(double S0, double SZ, double E0, double F0,
  ...
  return SZ * S0 * boltzman_ratio(T, T0, E0) * stimulated_relative_emission(stimulated_emission(T, F0), stimulated_emission(T0, F0)) * QT0 / QT;
}
             ...
             const Absorption::Zeeman::Polarization polarization)
{
  ...
  for (size_t iline=0; iline<band.n_lines(); iline++) {
    ...
    const auto zeeman_range = line.ZeemanRange(polarization, band.Isotopologue());
    const auto X = line.ShapeModel()(atm.atm.Temp(), band.T0(), atm.atm.Pres(), atm.atm.VolumeMixingRatios());
    const Complex lm{1.0 + X.G, - X.Y};
    
    for (int iz=zeeman_range.first; iz<=zeeman_range.second; iz++) {
      const auto DZ = line.ZeemanSplitting(polarization, band.Isotopologue(), iz);
      const auto SZ = line.ZeemanStrength(polarization, band.Isotopologue(), iz);
      ...
      switch (band.ShapeType()) {
        ...
        case Shape::SDVP:
          compute_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
          LineShape::Base::SpeedDependentVoigt(line.F0(), X, GDpart, DZ * H));
          break;
        case Shape::SDHCVP:
          compute_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
          LineShape::Base::SpeedDependentHardCollisionVoigt(line.F0(), X, GDpart, DZ * H));
          break;
        case Shape::HTP:
          compute_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
      ...
      switch (band.MirrorType()) {
        case Mirroring::Same:
          switch (band.ShapeType()) {
            case Shape::DP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::Doppler(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::LP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::Lorentz(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::VP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::Voigt(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::SDVP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::SpeedDependentVoigt(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::SDHCVP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::SpeedDependentHardCollisionVoigt(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::HTP:
              compute_mirrored_lineshape(comp_x, f, lm, cutoff_low, cutoff_upp,
              LineShape::Base::HartmannTran(-line.F0(), mirrored(X), -GDpart, DZ * H));
              break;
            case Shape::FINAL: {/* leave last */}
          } break;
...
//           apply_rosenkranz_quadratic_scaling(F, dF, f, band.F0(i), T, band, i, derivatives_data, derivatives_data_active);
      ...
      switch (band.PopType()) {
        case Population::ByLTE: {
            const double S = compute_lte_linestrength(line.I0(), SZ, line.E0(), line.F0(),
                                                      QT0, band.T0(), QT, atm.atm.Temp());
            std::transform(std::execution::par_unseq, comp_x.begin(), comp_x.end(), comp_x.begin(), [S](auto& a){return S*a;});
        } break;
      ...
      std::transform(std::execution::par_unseq, comp_x.cbegin(), comp_x.cend(), x.begin(), x.begin(), [](auto& a, auto& b){return a+b;});
    }
  }
}
}  // Xsec
