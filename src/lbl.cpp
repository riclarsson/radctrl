...
std::vector<Band> parse_hitran_with_qns(File::File<File::Operation::Read, File::Type::Raw>& hitranfile,
  ...
  // Get the HITRAN data
  std::vector<std::vector<HITRAN::IsotopologueData>> hitrandata{int(Species::Species::FINAL)};
  ...
  while (not hitranfile.at_end()) {
    ...
    auto s = Species::Isotope{Species::Isotope(hitrandata[hitpar.M][hitpar.I-1].spec, hitrandata[hitpar.M][hitpar.I-1].isotpos)};
    ...
    for (size_t i=0; i<quantums[0].size(); i++) {
      s.set_global(gu,quantums[0][i],HITRAN::toQuantumType(HITRAN::QuantumTypes(i)));
      s.set_global(gl,quantums[1][i],HITRAN::toQuantumType(HITRAN::QuantumTypes(i)));
      s.set_local(lu,quantums[0][i],HITRAN::toQuantumType(HITRAN::QuantumTypes(i)));
      s.set_local(ll,quantums[1][i],HITRAN::toQuantumType(HITRAN::QuantumTypes(i)));
    }
    
    Band b{s, Mirroring::None, Normalization::None, Population::ByLTE, Cutoff::None,
           Shape::VP, 296, -1, gu, gl};
    ...
    Line l{s, hitpar.v, hitpar.S, hitpar.Elow,
      Zeeman{}, hitpar.gupp, hitpar.glow, hitpar.A,
            ll, lu, LineShape::Model{s.Spec(),
              hitpar.gamma_air, hitpar.gamma_self,
...
void saveBand(File::File<File::Operation::Write, File::Type::Xml>& file, const Band& band, const std::string& key) {
  ...
  for (size_t i=0; i<gqn.size(); i++) {
    globalqn << ' ' << gqn[i] << ' ' << band.globalUpperQuantumNumber(i) << ' '<< band.globalLowerQuantumNumber(i);
  }
  ...
  for (auto& x: band.Lines()) {
    ...
    for (size_t i=0; i<lqn.size(); i++) {
      if (i)
        file << ' ';
      file << x.localUpperQuantumNumber(i) << ' '<< x.localLowerQuantumNumber(i);
    }
    file << '\n';
  }
...
void readBand(File::File<File::Operation::Read, File::Type::Xml>& file, Band& band, const std::string& key) {
  auto c = file.get_child(key.c_str());
  
  Species::Species spec = Species::toSpecies(file.get_attribute("Species").as_string());
  ...
  band.normalization = toNormalization(file.get_attribute("Normalization").as_string());
  ...
  std::stringstream globalqn(file.get_attribute("GlobalQuantumNumbers").as_string());
  int ngqn;
  globalqn >> ngqn;
  ...
  std::stringstream localqn(file.get_attribute("LocalQuantumNumbers").as_string());
  int nlqn;
  localqn >> nlqn;
  ...
  band.lines = std::vector<Line>(file.get_attribute("N").as_ullong(), {band.spec});
...
void saveBand(File::File<File::Operation::WriteBinary, File::Type::Xml>& file, const Band& band, const std::string& key) {
  file.new_child(key.c_str());
  ...
  for (auto& x: band.Lines()) {
    ...
    for (size_t i=0; i<model.n_spec(); i++)
      file.write(model[i]);
...
void readBand(File::File<File::Operation::ReadBinary, File::Type::Xml>& file, Band& band, const std::string& key) {
  ...
  for (Line& line: band.lines) {
    ...
    for (size_t i=0; i<line.model.n_spec(); i++)
      file.read(line.model[i]);
