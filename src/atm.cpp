...
namespace Interp {
template <size_t N> constexpr double sum(const std::array<double, N>& vals) {return std::accumulate(vals.cbegin(), vals.cend(), 0);}
                      ...
                      Coordinate<CoordinateType::lon> newlon) const
{
  ...
  if (lonw not_eq 1) {
    ...
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin(), latlow-lat.cbegin()+1, lonlow-lon.cbegin() ), (tidw)*(altw)*(1-latw)*(lonw)}; // 0010
    
  } if (latw not_eq 1 and lonw not_eq 1) {
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin(), latlow-lat.cbegin()+1, lonlow-lon.cbegin()+1), (tidw)*(altw)*(1-latw)*(1-lonw)}; // 0011
    
  } if (altw not_eq 1) {
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin()+1, latlow-lat.cbegin(), lonlow-lon.cbegin() ), (tidw)*(1-altw)*(latw)*(lonw)}; // 0100
    
  } if (altw not_eq 1 and lonw not_eq 1) {
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin()+1, latlow-lat.cbegin(), lonlow-lon.cbegin()+1), (tidw)*(1-altw)*(latw)*(1-lonw)}; // 0101
    
  } if (altw not_eq 1 and latw not_eq 1) {
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin()+1, latlow-lat.cbegin()+1, lonlow-lon.cbegin() ), (tidw)*(1-altw)*(1-latw)*(lonw)}; // 0110
    
  } if (altw not_eq 1 and lonw not_eq 1 and latw not_eq 1) {
    out += {data(tidlow-tid.cbegin(), altlow-alt.cbegin()+1, latlow-lat.cbegin()+1, lonlow-lon.cbegin()+1), (tidw)*(1-altw)*(1-latw)*(1-lonw)}; // 0111
    
  } if (tidw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin(), latlow-lat.cbegin(), lonlow-lon.cbegin() ), (1-tidw)*(altw)*(latw)*(lonw)}; // 1000
    
  } if (tidw not_eq 1 and lonw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin(), latlow-lat.cbegin(), lonlow-lon.cbegin()+1), (1-tidw)*(altw)*(latw)*(1-lonw)}; // 1001
    
  } if (tidw not_eq 1 and latw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin(), latlow-lat.cbegin()+1, lonlow-lon.cbegin() ), (1-tidw)*(altw)*(1-latw)*(lonw)}; // 1010
    
  } if (tidw not_eq 1 and latw not_eq 1 and lonw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin(), latlow-lat.cbegin()+1, lonlow-lon.cbegin()+1), (1-tidw)*(altw)*(1-latw)*(1-lonw)}; // 1011
    
  } if (tidw not_eq 1 and altw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin()+1, latlow-lat.cbegin(), lonlow-lon.cbegin() ), (1-tidw)*(1-altw)*(latw)*(lonw)}; // 1100
    
  } if (tidw not_eq 1 and altw not_eq 1 and lonw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin()+1, latlow-lat.cbegin(), lonlow-lon.cbegin()+1), (1-tidw)*(1-altw)*(latw)*(1-lonw)}; // 1101
    
  } if (tidw not_eq 1 and altw not_eq 1 and latw not_eq 1) {
    out += {data(tidlow-tid.cbegin()+1, altlow-alt.cbegin()+1, latlow-lat.cbegin()+1, lonlow-lon.cbegin() ), (1-tidw)*(1-altw)*(1-latw)*(lonw)}; // 1110
