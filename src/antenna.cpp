...
std::vector<Sensor::Antenna::Output> calc_airy_disk(
  ...
  out.push_back(Sensor::Antenna::Output{
  ...
  for (std::size_t i = 1; i < num; i++) {
    for (std::size_t j = 0; j < azimuthals; j++) {
      ...
      out.push_back(
          {Path::calc_single_geometric_path(new_pos_los, atm, dist, alt_of_atm),
           invsumy_azimuth * y[i]});
    }
  }
...
std::vector<Sensor::Antenna::Output> Sensor::Antenna::calc(
  ...
  switch (mtype) {
    case BeamType::PencilBeam:
      return std::vector<Sensor::Antenna::Output>(
          1, Output{Path::calc_single_geometric_path(pos_los, atm, dist,
                                                     alt_of_atm),
                    1});
