...
class Grid {
 ...
 public:
  Grid(const base& fillval, std::array<std::size_t, N> size)
      : gridsize(size),
        d(std::reduce(gridsize.cbegin(), gridsize.cend(), 1,
  ...
  void reset(std::size_t I, std::size_t J, std::size_t K, std::size_t M,
             const base& fillval) {
    gridsize = {I, J, K, M};
    d = std::vector<base>(std::reduce(gridsize.cbegin(), gridsize.cend(), 1,
                                      std::multiplies<std::size_t>{}),
                          fillval);
  }
