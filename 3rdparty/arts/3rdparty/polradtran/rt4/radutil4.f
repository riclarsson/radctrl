...
C Added by JM for use with ARTS.
      SUBROUTINE SPECULAR_SURFACE (NSTOKES, NUMMU,
     .                             GROUND_REFLEC, 
     .                             REFLECT, TRANS, SOURCE)
C         SPECULAR_SURFACE makes the reflection matrix for a
C      plane surface with fixed reflectivity.  Also makes the diagonal
C      transmission and zero source function.
      INTEGER NSTOKES, NUMMU
      ...
      DO J = 1, NUMMU
        DO S1 = 1, NSTOKES
          DO S2 = 1, NSTOKES
            REFLECT(S1,J, S2,J, 2) = GROUND_REFLEC(S2,S1)
          ENDDO
        ENDDO
      ENDDO
...
C Added by JM for use with ARTS.
      SUBROUTINE SPECULAR_RADIANCE (NSTOKES, NUMMU, 
     .                              GROUND_REFLEC, GROUND_TEMP,
     .                              WAVELENGTH, RADIANCE)
C        SPECULAR_RADIANCE calculates the ground radiance of a plane
C      surface with fixed reflectivity.  The radiance is due only to
C      thermal radiation (this subroutine cannot do specular reflection).
      INTEGER NSTOKES, NUMMU
      REAL*8  GROUND_REFLEC(NSTOKES,NSTOKES)
...
C           Thermal radiation going up
      ...
      CALL PLANCK_FUNCTION (GROUND_TEMP, 'R', WAVELENGTH, PLANCK)
      DO J = 1, NUMMU
          RADIANCE(1,J) = (1.0 - GROUND_REFLEC(1,1)) * PLANCK
          IF (NSTOKES .GT. 1) RADIANCE(2,J)= -GROUND_REFLEC(1,2)*PLANCK
      ENDDO
...
C Added by JM for use with ARTS.
      SUBROUTINE EXTERNAL_SURFACE (NSTOKES, NUMMU,
     .                             SURF_REFL, RADIANCE, 
     .                             REFLECT, TRANS, SOURCE)
C         EXTERNAL_REFLECT sets the reflection matrix for the
C      surface from externally prepared data, just copying over the data
C      into the right part of REFLECT.  Also makes the diagonal
C      transmission and zero source function.
      INTEGER NSTOKES, NUMMU
      REAL*8  SURF_REFL(NSTOKES,NUMMU, NSTOKES,NUMMU)
      REAL*8  RADIANCE(NSTOKES,NUMMU)
      REAL*8  REFLECT(NSTOKES,NUMMU, NSTOKES,NUMMU, 2)
      REAL*8  TRANS(NSTOKES,NUMMU, NSTOKES,NUMMU, 2)
      REAL*8  SOURCE(NSTOKES,NUMMU, 2)
...
C Added by JM for use with ARTS.
      SUBROUTINE PRINTOUT ( GROUND_TYPE, NSTOKES, NUMMU,
     .                      REFLECT, RADIANCE )
      INTEGER NSTOKES, NUMMU
      REAL*8  RADIANCE(NSTOKES,NUMMU)
      REAL*8  REFLECT(NSTOKES,NUMMU, NSTOKES,NUMMU, 2)
      ...
      WRITE(*,'(A,A1)') 'Ground type: ', GROUND_TYPE

      DO J1 = 1, NUMMU
        WRITE(*,'(A,I3,A,2E12.5)') 'GNDRAD(',J1,') = ',
     .                             (RADIANCE(N,J1), N=1,NSTOKES)
      ENDDO

      DO J1 = 1, NUMMU
        DO J2 = 1, NUMMU
          WRITE(*,'(A,2I3,A,4E12.5)') 'REFLEC(',J1,J2,') = ',
     .                                (REFLECT(N/NSTOKES,J1,
     .                                 N-N/NSTOKES,J2,2),
     .                                 N=1,NSTOKES*NSTOKES)
        ENDDO
      ENDDO
