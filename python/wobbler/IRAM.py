...
class wobbler:
    ...
    def _wait_until_motionless(self, t=None):
        ...
        while stat & 0x1:
            ...
            time.sleep(0.125*abs(self._pos-int(resp))/float(f) if (t is None) else t)
        return 0
...
#        print(message)
    ...
    def _read(self,timeout=.5):
        ...
        t=time.time()
        while self._serial.in_waiting==0 and time.time()-t<timeout: time.sleep(.1)
